...
import { AllCategory, Cover_Image, Datasheet_Catalogues, Image_Catalogues, Product, Size, Specification } from "@prisma/client"
...
export const ProductForm: React.FC<ProductFormProps> = ({
  ...
  const onSubmit = async (data: ProductFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/${params.brandId}/products/${params.productId}`, data);
      } else {
        await axios.post(`/api/${params.brandId}/products`, data);
      }
      router.refresh();
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/${params.brandId}/products/${params.productId}`);
      router.refresh();
      router.push(`/${params.brandId}/products`);
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                  ...
                  <div className="space-y-1 leading-none">
                    ...
                    <FormDescription>
                      This product will not appear anywhere in the brand.
                    </FormDescription>
                  </div>
                </FormItem>
              )}
